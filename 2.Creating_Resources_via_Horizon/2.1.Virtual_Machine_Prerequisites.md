...
Next, the final piece for connecting the two networks. While in the Network Topology view, hover the cursor over box for the router just created, click on "+ Add Interface" button, and select the private subnet in the "Subnet" field:
<p float="left">
...
Our updated Network Topology view:
<p align="center">
  <img width="60%" alt="network_topology_router_interface_added" src="https://github.com/user-attachments/assets/d8c7eea4-21f3-4c00-9ae3-899f66ae8207" />
</p>
...
### SSH Key Pair
By default, most Cloud OS images do not have a default password for the default user. One way to get around this is to use an SSH key pair. Navigate to the "Compute" --> "Key Pairs" section on the left menu, and either click on "+ Create Key Pair" or "Import Public Key" the top right. The simplest (however not necessarily the most secure) is to create a pair:
<p align="center">
  <img width="60%" alt="create_ssh_keypair" src="https://github.com/user-attachments/assets/c2f76798-84b6-4d64-bbb5-c74437d7a0cd" />
</p>
Note that the private key associated with the created public key will be downloaded automatically to your computer -- in this case, a file called "my-ssh-key.pem".


Finally, we are ready to launch our first instance.
