# 4. Creating Resources via Terraform and GitLab

## 4.1. Creating a VM with Persistent Storage

Assuming we satisfy the [pre-requisites for creating a VM in our environment](../2-Creating_Resources_via_Horizon/2.1.Virtual_Machine_Prerequisites.md), we can proceed with this example that serves to illustrate the concept of creating an OpenStack instance via the infrastructure as code (IAC) approach. Furthermore, to make things a bit easier, we can draw upon a [pre-defined module for an instance](https://github.com/sfebruary/terraform-module-openstack-instance) that does most of the heavy lifting.

Begin by [creating a new, blank project in GitLab](https://docs.gitlab.com/user/project/#create-a-blank-project). 
...
```
**settings.tfvars** (modify as needed for your environment):
```bash
...
To interact with OpenStack services via Terraform, or more generally via the command-line interface (CLI), environment variables must be configured. These variables store authentication credentials and other details required for such interaction. The most common method is using an OpenStack RC file (openrc.sh), which can be downloaded from the landing page after logging into the Horizon dashboard:
...
- Next, you will want to [install the Terraform CLI](https://developer.hashicorp.com/terraform/tutorials/aws-get-started/install-cli).
- Optionally, you can also [install the OpenStack CLI](https://pypi.org/project/python-openstackclient/), useful for sanity checking and querying when used in combination with Terraform.
...
- After successfully initialising, you will then execute
  ```bash
  terraform plan -var-file=settings.tfvars
  ```
- Assuming everything looks good with the above (NB! you have the freedom to modify **settings.tfvars** to customise your specific environment), proceed to running
  ```bash
  terraform apply -var-file=settings.tfvars
  ```
  and type "yes" at the prompt to proceed to create your first instance via Terraform.

**Challenges**
- Can you figure out two different ways to access this particular instance?
- This VM has access to a persistent storage volume, but it is not quite ready to be used by the VM: can you figure out the necessary steps to make it useable?
